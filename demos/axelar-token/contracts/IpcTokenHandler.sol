...
import {SubnetID, Asset, AssetKind} from "@consensus-shipyard/ipc-contracts/contracts/structs/Subnet.sol";
...
interface SubnetActor {
    function supplySource() external returns (Asset memory supply);
}
...
contract IpcTokenHandler is InterchainTokenExecutable, IIpcHandler, Ownable {
    ...
    function handleIpcMessage(IpcEnvelope calldata envelope) external payable returns (bytes memory ret) {
        ...
        if (result.outcome != OutcomeType.Ok) {
            ...
            Asset memory supplySource = SubnetActor(envelope.from.subnetId.getAddress()).supplySource();
            require(supplySource.kind == AssetKind.ERC20, "expected ERC20 supply source");
